...
class Places(torch.utils.data.Dataset):
    ...
    def _construct_db(self):
        ...
        self._db = []
        with open(os.path.join(self._data_path, 'places365_retrieval.pkl'), 'rb') as fin:
            gnd = pkl.load(fin)
            if self._split == 'query':
                for i in range(len(gnd["qimlist"])):
                    im_fn = gnd["qimlist"][i][:-1]
    ...
    def __getitem__(self, index):
        ...
        if "class" in self._db[index].keys():
            return im, int(self._db[index]["class"]), self._db[index]["idx"]
        
        return im, int(self._db[index]["qclass"]), self._db[index]["idx"]
